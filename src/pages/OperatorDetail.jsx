...
import { getOperatorDetails, getRiskBreakdown } from '../../api/operatorApi';
...
const OperatorDetail = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true);
                const [detailsData, breakdownData] = await Promise.all([
                    getOperatorDetails(operatorId),
                    getRiskBreakdown(operatorId)
                ]);
    ...
    const renderContent = () => {
        if (loading) {
            return <Loader text={`Loading details for ${operatorId}...`} />;
        }
        ...
        return (
            <div className="space-y-8">
                {/* Header Section */}
                <motion.div variants={itemVariants} className="bg-white shadow-sm rounded-lg p-6">
                    <div className="flex flex-wrap justify-between items-start">
                        <div>
                            ...
                            <p className="mt-1 text-gray-600">{details.district}</p>
                        </div>
                        <div className="text-right mt-4 sm:mt-0">
                            <p className="text-sm font-medium text-gray-500">Risk Score</p>
                            <div className="flex items-center gap-x-3">
                                <h2 className="text-3xl font-bold text-gray-900">{details.riskScore.toFixed(1)}</h2>
                                <RiskBadge level={details.riskLevel} />
                            </div>
                            <p className="mt-1 text-xs text-gray-500">Last Updated: {new Date(details.lastUpdated).toLocaleString()}</p>
                        </div>
                    </div>
                </motion.div>
                ...
                {/* Metrics Radar Chart */}
                <motion.div variants={itemVariants} className="bg-white shadow-sm rounded-lg p-6">
                    <h2 className="text-lg font-medium text-gray-900">Performance Metrics vs. District Average</h2>
                    <p className="text-sm text-gray-500 mb-4">A comparison of key performance indicators.</p>
                    <OperatorMetricsRadar operatorMetrics={details.metrics} districtAverage={details.districtAverage} />
                </motion.div>

                {/* Risk Flags */}
                <motion.div variants={itemVariants} className="bg-white shadow-sm rounded-lg">
